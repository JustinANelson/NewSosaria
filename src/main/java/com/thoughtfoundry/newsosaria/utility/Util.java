package com.thoughtfoundry.newsosaria.utility;
...
public class Util {
    ...
    public static void registerTestGuildParameters(){
        ...
        Guild guild = NewSosaria.jda.getGuildById(AppConfig.SUPPORTSERVER);

        if (guild != null) {
            guild.updateCommands().addCommands(BotCollections.guildCommands).queue();
        }
    }
    public static void registerGlobalCommands(){
        NewSosaria.jda.updateCommands().addCommands(new ArrayList<CommandData>()).queue();
    }
